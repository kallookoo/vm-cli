...
Command to simplify the use of VirtualBox (VBoxManage) or VMWare (vmrun) from the terminal.
...
```shell
(
    curl -sfL "https://github.com/kallookoo/vm-cli/releases/download/v2.0.0/vm-cli.virtualbox.tar.gz" | \
...
```shell
(
    curl -sfL "https://github.com/kallookoo/vm-cli/releases/download/v2.0.0/vm-cli.vmware.tar.gz" | \
    tar -xzf - -C "$HOME/.local/bin" && chmod u+x "$HOME/.local/bin/vm-cli"
)
```
